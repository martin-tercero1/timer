    ...
    <h1 class=hero--title>Timer</h1>
      ...
      <div class="hero--buttons">
        <form onsubmit="startTimer()">
            <input type="number" placeholder="Write the minutes" id="minutesInput" name="minutes" ></input>
            <input type="number" placeholder="Write the seconds" id="secondsInput" name="seconds" ></input>
...
function startPomodoro() {
    ...
    currentInterval = setInterval(() => {
        ...
        if (minutesValue === 0 && secondsValue === 0) {
            ...
            title.textContent = "The timer has finished";
...
function startTimer() {
    ...
    currentInterval = setInterval(() => {
        ...
        if (minutesValue === 0 && secondsValue === 0) {
            const container = document.querySelector(".hero--time");
            const title = document.createElement("h2");
            document.title = "The pomodoro has finished";
            title.textContent = "The pomodoro has finished";
            alert("The pomodoro has finished");
            container.appendChild(title);
            clearInterval(currentInterval);
        }
        minutesSpan.textContent = formatValue(minutesValue);
